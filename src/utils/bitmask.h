...
class Bitmask : public Bits {
	...
	public:
		...
		Bitmask(T const&& val) noexcept; 			   /* const integral type for e.g Bitmask<Bits>::opt1 */

		template <typename T>
		explicit Bitmask(T const&& val) noexcept;    /* Other types require explicit conversion */
