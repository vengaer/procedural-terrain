...
GLuint Shader::init(std::string const& shader1, Type type1, std::string const& shader2, Type type2){
	...
	try{
		shader_ids[static_cast<unsigned>(type1)] = compile(read_source(shader1), type1);
	}
	catch(ShaderCompilationException const& err){
		glDeleteShader(shader_ids[static_cast<unsigned>(type1)]);
		throw;
	}
	try{
	...
	return link(shader_ids[static_cast<unsigned>(Type::VERTEX)], shader_ids[static_cast<unsigned>(Type::FRAGMENT)]);
}
